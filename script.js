...
const elements = {
  ...
  mobileInventoryModal: document.querySelector("#mobileInventoryModal"),
  mobileInventoryCloseButton: document.querySelector("#mobileInventoryCloseButton"),
  mobileInventoryButton: document.querySelector("#mobileInventoryButton"),
  battleMobileInventoryButton: document.querySelector("#battleMobileInventoryButton"),
  mobileInventoryItems: document.querySelector("#mobileInventoryItems"),
  ...
  mobileGold: document.querySelector("#mobileGold"),
  cost: document.querySelector("#cost"),
  mobileCost: document.querySelector("#mobileCost"),
  chance: document.querySelector("#chance"),
  mobileChance: document.querySelector("#mobileChance"),
...
function getAuthErrorMessage(error) {
  ...
  if (code.includes("operation-not-allowed")) return "Firebase 콘솔에서 이메일/비밀번호 로그인을 켜야 합니다.";
  ...
  if (code.includes("user-not-found") || code.includes("invalid-credential") || code.includes("wrong-password")) return "이메일 또는 비밀번호가 맞지 않습니다.";
...
async function loginWithEmail() {
  ...
  if (!email || !password) {
    elements.authHint.textContent = "이메일과 비밀번호를 입력하세요.";
    return;
  }

  elements.authHint.textContent = "메일 로그인 중...";
  try {
    await signInWithEmailAndPassword(firebaseAuth, email, password);
    elements.authHint.textContent = "메일 로그인 완료. 저장 데이터를 확인 중입니다.";
  } catch (error) {
...
async function signupWithEmail() {
  ...
  elements.authHint.textContent = "메일 회원가입 중...";
  try {
...
function setBattleMessage(text, type = "", { html = false } = {}) {
  if (html) {
    elements.battleMessage.innerHTML = text;
  } else {
...
function addLog(text, type = "", { html = false } = {}) {
...
function openMobileInventoryModal() {
  closeMobileSettings();
  renderInventory();
  elements.mobileInventoryModal.classList.remove("hidden");
}

function closeMobileInventoryModal() {
  elements.mobileInventoryModal.classList.add("hidden");
}
...
function renderInventory() {
  ...
  elements.mobileInventoryItems.innerHTML = html;
}
...
function render() {
  ...
  elements.mobileGold.innerHTML = formatGoldHtml(state.gold);
  elements.mobileCost.innerHTML = isMax ? "완료" : formatGoldHtml(cost);
  elements.mobileChance.textContent = isMax ? "100%" : formatPercent(chance);
...
function defeatMonster() {
  ...
  setBattleMessage(`${monsterNames[defeatedLevel - 1]} 처치! ${formatGoldHtml(earned)} 획득.${dropText}`, "clear", { html: true });
  addLog(`${defeatedLevel}단계 몬스터 처치: ${formatGoldHtml(earned)} 획득`, "success", { html: true });
...
function runAutoEnhanceStep() {
  ...
  if (state.level >= targetLevel) {
    stopAutoEnhance();
    const reason = `목표 +${targetLevel}에 도달했습니다.`;
  ...
  if (state.gold < cost) {
    stopAutoEnhance();
    const reason = "골드가 부족해 자동 강화를 중지했습니다.";
    setMessage(`${reason} 필요 골드: ${formatGold(cost)}`);
    showAutoEnhanceStopReason(reason);
    return;
  }
  ...
  if (!hasEnoughItemsForPlan(autoEnhanceItemPlan)) {
    stopAutoEnhance();
    const reason = "선택한 아이템이 부족해 자동 강화를 중지했습니다.";
...
function showItemDepletedBanner(text = "아이템을 모두 소진하셨습니다") {
  ...
  window.setTimeout(() => elements.itemDepletedBanner.classList.remove("show"), 3000);
}

function showAutoEnhanceStopReason(text) {
  showItemDepletedBanner(text);
}
...
elements.mobileInventoryButton.addEventListener("click", openMobileInventoryModal);
elements.battleMobileInventoryButton.addEventListener("click", openMobileInventoryModal);
...
elements.authLoginButton.addEventListener("click", loginWithEmail);
...
elements.mobileInventoryItems.addEventListener("click", (event) => {
  const button = event.target.closest("[data-item]");
  if (!button) return;
  selectInventoryItem(button.dataset.item);
});
...
elements.mobileInventoryCloseButton.addEventListener("click", closeMobileInventoryModal);
...
elements.mobileInventoryModal.addEventListener("click", (event) => {
  if (event.target === elements.mobileInventoryModal) closeMobileInventoryModal();
});
